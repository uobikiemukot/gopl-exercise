...
func main() {
	...
	for i := 0; i < cells; i++ {
		for j := 0; j < cells; j++ {
			...
			r = math.Max(r, 0.0)
			b = math.Max(b, 0.0)
			c := fmt.Sprintf("#%.2X00%.2X", int(r), int(b))
